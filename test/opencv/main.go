...
func main() {
	...
	var TOTAL_VEHICLESS int32 = 5

	for i := int32(0); i < TOTAL_VEHICLESS; i++ {
	...
	for i := int32(0); i < TOTAL_VEHICLESS; i++ {
		go func(i int32) {
			defer wg.Done()
			for j := int32(0); j < TOTAL_VEHICLESS; j++ {
				if i == j {
